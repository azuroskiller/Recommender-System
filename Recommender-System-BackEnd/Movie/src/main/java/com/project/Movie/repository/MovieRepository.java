...
public class MovieRepository {
    ...
    private String INSERT_MOVIE = "INSERT INTO HAIRUL_MOVIES ( TITLE, GENRES, POSTER, DESCRIPTION) VALUES (?, ?, ?, ?)";
    ...
    public List<Movies> findByID(int id){
        return jdbcTemplate.query("SELECT * FROM HAIRUL_MOVIES WHERE MOVIEID ="+id, rowMapper);
    }

    //Method to get user fav movies by passing the userID
    public List<Movies> getMyMovie(int id){
        return jdbcTemplate.query("SELECT * FROM HAIRUL_MOVIES WHERE MOVIEID IN ( SELECT MOVIEID FROM HAIRUL_FAV_MOVIES WHERE USERID = "+id+")", rowMapper);
    }
    ...
    public Page<Movies> findAllByPage(Pageable page){
        ...
        String rowCountSQL = "SELECT count(*) AS ROW_COUNT FROM HAIRUL_MOVIES";
        ...
        List<Movies> movies = jdbcTemplate.query("SELECT * FROM HAIRUL_MOVIES ORDER BY MOVIEID DESC"
    ...
    public boolean deleteMovie(Movies m){
        ...
        String sql = "DELETE FROM HAIRUL_MOVIES WHERE MOVIEID = ?";
