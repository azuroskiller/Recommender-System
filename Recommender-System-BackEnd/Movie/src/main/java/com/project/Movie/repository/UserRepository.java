...
public class UserRepository {
    ...
    private String INSERT_USER = "INSERT INTO HAIRUL_USER ( EMAIL, FIRSTNAME, LASTNAME, PASSWORD, USERNAME, TYPE) VALUES (?, ?, ?, ?, ?, "+type+")";
    ...
    public List<User> findAll(){
        return jdbcTemplate.query("Select * from HAIRUL_USER", rowMapper);
    }

    //Login method by passing the username and search by username
    public User findByUsername(String Username) {
        String sql = "SELECT * FROM HAIRUL_USER WHERE USERNAME LIKE '";
    ...
    public User findByEmail(String Email) {
        String sql = "SELECT * FROM HAIRUL_USER WHERE EMAIL LIKE '";
    ...
    {
        String sql = "UPDATE HAIRUL_USER SET TYPE = 1 WHERE USERID = ?";
    ...
    {
        String sql = "DELETE FROM HAIRUL_USER WHERE USERID = ?";
        if ( jdbcTemplate.update(sql, u.getUSERID()) > 0) return true;
        else
            return false;
    }
